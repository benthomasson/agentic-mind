...
# Import beliefs and nogoods from file-based tools
shared-enterprise import-beliefs beliefs.md
shared-enterprise import-nogoods nogoods.md

# Check index status
shared-enterprise status
```

## Full Sync (index + beliefs + nogoods)

```bash
# One command — indexes entries, imports beliefs, imports nogoods
shared-enterprise sync

# With options
shared-enterprise sync --reindex                    # force re-index all files
shared-enterprise sync --entries src/ --beliefs claims.md  # custom paths
...
3. Import beliefs: `shared-enterprise import-beliefs beliefs.md`
4. Import nogoods: `shared-enterprise import-nogoods nogoods.md`
5. Use `shared-enterprise context "topic"` for multi-source retrieval
6. Distill key findings into claims with `shared-enterprise claims add`
7. Run `shared-enterprise claims audit` periodically to check health
...
- "sync the database" → `shared-enterprise sync`
